...
Clingo5_4::Clingo5_4(const std::string& incrementalVar,
...
struct RuleToString5_4 {
  RuleToString5_4(unsigned int timeStepNum) {
    stringstream ss;
    ss << timeStepNum;
    timeStep = ss.str();
  }

  RuleToString5_4(const std::string& timeStepVar) : timeStep(timeStepVar) {}

  RuleToString5_4() : timeStep("") {}
...
struct RuleToCumulativeString5_4 {

  RuleToCumulativeString5_4(const std::string& timeStepVar) : timeStep(timeStepVar) {}
...
struct RuleToGoalString5_4 {

  RuleToGoalString5_4(const std::string& timeStepVar) : timeStep(timeStepVar) {}
...
static string cumulativeString(const std::vector<actasp::AspRule>& query, const string& timeStepVar) {
  ...
  transform(query.begin(),query.end(),ostream_iterator<std::string>(aspStream),RuleToCumulativeString5_4(timeStepVar));
...
static string aspString(const std::vector<actasp::AspRule>& query, const string& timeStepVar) {
  ...
  stringstream aspStream;
  transform(query.begin(),query.end(),ostream_iterator<std::string>(aspStream),RuleToString5_4(timeStepVar));
  return aspStream.str();
}
...
string Clingo5_4::generatePlanQuery(std::vector<actasp::AspRule> goalRules) const throw() {
  stringstream goal;
  ...
  transform(goalRules.begin(),goalRules.end(),ostream_iterator<std::string>(goal),RuleToGoalString5_4(incrementalVar));
...
std::list<actasp::AnswerSet> Clingo5_4::minimalPlanQuery(const std::vector<actasp::AspRule>& goalRules,
    bool filterActions,
...
struct MaxTimeStepLessThan5_4 {

  MaxTimeStepLessThan5_4(unsigned int initialTimeStep) : initialTimeStep(initialTimeStep) {}
...
std::list<actasp::AnswerSet> Clingo5_4::lengthRangePlanQuery(const std::vector<actasp::AspRule>& goalRules,
    bool filterActions,
  ...
  allplans.remove_if(MaxTimeStepLessThan5_4(min_plan_length));
...
actasp::AnswerSet Clingo5_4::optimalPlanQuery(const std::vector<actasp::AspRule>& goalRules,
    bool filterActions,
    unsigned int  max_plan_length,
...
AnswerSet Clingo5_4::currentStateQuery(const std::vector<actasp::AspRule>& query) const throw() {
  list<AnswerSet> sets = genericQuery(aspString(query,0),0,0,"stateQuery",1,true);

  return (sets.empty())? AnswerSet() : *(sets.begin());
}

struct HasTimeStepZeroInHead5_4 : unary_function<const AspRule&,bool> {
...
std::list<actasp::AnswerSet> Clingo5_4::genericQuery(const std::vector<actasp::AspRule>& query,
    unsigned int timeStep,
    const std::string& fileName,
    unsigned int answerSetsNumber) const throw() {

  std::vector<actasp::AspRule> base;
  remove_copy_if(query.begin(),query.end(),back_inserter(base), not1(HasTimeStepZeroInHead5_4()));
  ...
  remove_copy_if(query.begin(),query.end(),back_inserter(cumulative), HasTimeStepZeroInHead5_4());
...
std::string Clingo5_4::generateMonitorQuery(const std::vector<actasp::AspRule>& goalRules,
...
std::list<actasp::AnswerSet> Clingo5_4::monitorQuery(const std::vector<actasp::AspRule>& goalRules,
    const AnswerSet& plan) const throw() {
  ...
  result.remove_if(MaxTimeStepLessThan5_4(plan.getFluents().size()));
...
std::string Clingo5_4::makeQuery(const std::string& query,
                                 unsigned int initialTimeStep,
...
std::list<actasp::AnswerSet> Clingo5_4::genericQuery(const std::string& query,
    unsigned int initialTimeStep,
    unsigned int finalTimeStep,
    const std::string& fileName,
    unsigned int answerSetsNumber,
...
std::list< std::list<AspAtom> > Clingo5_4::genericQuery(const std::string& query,
    unsigned int timestep,
    const std::string& fileName,
    unsigned int answerSetsNumber) const throw() {
...
void Clingo5_4::setCurrentState(const std::set<actasp::AspFluent>& newState) {
...
std::list<actasp::AnswerSet> Clingo5_4::filteringQuery(const AnswerSet& currentState, const AnswerSet& plan,const std::vector<actasp::AspRule>& goals) {
